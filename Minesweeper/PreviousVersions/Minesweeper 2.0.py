...
from loadImages import load, Transform # Load Image Module
...
class Board():
    ...
    def __init__(self, boardSize, Mines, startTile):
...
class HUD():
    ...
    def _DisplayClock(self, started):
...
while MainGameLoop:
    ...
    startTile = []
    Game = Board(boardSize, int(Mines), startTile)
    ...
    # Creating the GameBoard Map AFTER INITIAL CLICK
    ...
    while GameLoop:
        ...
        Status, TileNum, clicks, FirstClick = RemoveTiles(AllTiles, FlagLock, Hud, Game, clicks)

        if FirstClick == True:
        
            Game._startTile = [TileNum[0], TileNum[1]]
            Game._placeMines()
            Game._bombAlerts()
